...
inline bool convrot_q8_decomp_backend_safe(const char* backend_name) {
    // Metal's Q8_0 decomposition remains finite on smoke shapes but produces
    // NaNs on the full MiniMax-H3 diffusion trajectory at sampler step 2.
    return backend_name == nullptr || std::strncmp(backend_name, "MTL", 3) != 0;
}
...
inline String2TensorStorage select_convrot_tensor_storage(ggml_backend_t backend,
    ...
    const bool allow_unsafe_metal_q8 = std::getenv("SD_CONVROT_Q8_ALLOW_UNSAFE_METAL") != nullptr;
    if (q8_decomp_mode &&
        (convrot_q8_decomp_backend_safe(backend_name) || allow_unsafe_metal_q8)) {
    ...
    if (q8_decomp_mode) {
        LOG_WARN("ConvRot: Q8_0 decomposition is unsafe at full shape on backend %s; "
                 "falling back to the native compact operation for %s",
...
protected:
    ...
    ggml_backend_t vae_fallback_backend    = nullptr;    // borrowed (SDBackendManager-owned)
    ...
    size_t get_pending_runtime_params_size() const {
        if (params_ctx == nullptr || vae_fallback_backend == nullptr ||
            vae_fallback_backend == runtime_backend) {
            return 0;
        }
        size_t bytes = 0;
...
public:
    ...
    void set_vae_auto_cpu_fallback(bool enabled,
                                   float memory_ratio = 0.9f,
                                   ggml_backend_t fallback_backend = nullptr) {
        vae_auto_cpu_fallback_enabled = enabled;
        vae_fallback_backend          = fallback_backend;
        ...
        if (sd_backend_is_cpu(runtime_backend)) {
            LOG_INFO("%s VAE runtime is explicitly CPU; automatic fallback is not needed",
                     get_desc().c_str());
        } else if (vae_fallback_backend == nullptr ||
                   !sd_backend_is_cpu(vae_fallback_backend)) {
            vae_auto_cpu_fallback_enabled = false;
            LOG_WARN(
                ...
                get_desc().c_str(),
                vae_fallback_backend != nullptr
                    ? ggml_backend_name(vae_fallback_backend)
                    : "(none)");
        } else {
