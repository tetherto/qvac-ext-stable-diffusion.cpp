...
grep -q "model metadata validated"                       "$log" || fail "model metadata validation did not complete"
grep -qi "wrong shape\|not in model file"                "$log" && fail "tensor shape/name mismatch on load"
grep -Eqi "batch generation is unsupported|not supported by batch" "$log" || fail "missing the guarded-rejection error"
...
set -e
grep -Eqi "batch generation is unsupported|not supported by batch" "$log" || fail "generate_image() not rejected for ABot-World"
...
if [[ -n "${WAN_DIT:-}" ]]; then
  ...
  grep -Eqi "batch generation is unsupported|not supported by batch" "$log" && fail "stock Wan wrongly rejected (regression!)"
